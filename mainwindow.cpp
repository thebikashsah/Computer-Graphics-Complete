...
{
    ans.push_back({p1.x(),p1.y()});
    ans.push_back({p2.x(),p2.y()});
     if(scanline)
        DDAlineUtil(200,200,200);
    else
        DDAline(255,255,0);
}
...
void MainWindow::DDAlineUtil(int r, int g, int b) {
    double x1 = p1.x()/gridsize;
    double y1 = p1.y()/gridsize;
    double x2 = p2.x()/gridsize;
    double y2 = p2.y()/gridsize;
...
void MainWindow::on_lineclipping_clicked()
{
    ...
    for(int i=0;i<(int)ans.size();i+=2){
        int x11=ans[i].first;
        int y11=ans[i].second;
        int x22=ans[i+1].first;
        int y22=ans[i+1].second;
        p1.setX(x11);
        p1.setY(y11);
        p2.setX(x22);
        p2.setY(y22);
        DDAline(0,0,0);
    //    delay(1000);
        cohenSutherlandClip(x11,y11,x22,y22);
...
void MainWindow::on_reflection_2_clicked()
{
//    int angle = ui->anglespinbox->value();
    gridsize = ui->gridspinbox->value();
...
//    double cosang=cos(dang);
    ...
    for(int i=0;i<len;i++){
        vertex_list[i].second=(old_vertex[i].second+ui->frame->height()/2);
    }
...
//        coord[2]=1;
//        mat_mult(mat,coord,res);
//        vertex_list[i].first=res[0];
//        vertex_list[i].second=-res[1];
//    }
